$(function () {
  ...
  function handleUserLayoutSetting() {
    if (
      typeof cookie_not_handle_user_settings != "undefined" &&
  ...
  //Collapse button
  $("#sidebar.sidebar-collapsed #sidebar-collapse > i").attr(
    "class",
  ...
  $('.mail-toolbar > li:first-child > input[type="checkbox"]').change(
    function () {
      ...
      $(this)
        .parents(".mail-content")
        .find('.mail-list .ml-left > input[type="checkbox"]')
        .prop("checked", check);
  ...
  if ($(".tile-active").size() > 0) {
    ...
    var tileGoUp = function (el, stop1, stop2, height) {
      $(el)
        .children(".tile")
        .animate({ top: "-=" + height + "px" }, tileMoveDuration);
      setTimeout(function () {
        tileGoDown(el, stop1, stop2, height);
      }, stop2 + tileMoveDuration);
    };
    ...
    var tileGoDown = function (el, stop1, stop2, height) {
      $(el)
        ...
        .animate({ top: "+=" + height + "px" }, tileMoveDuration);
      setTimeout(function () {
        tileGoUp(el, stop1, stop2, height);
      }, stop1 + tileMoveDuration);
    };
  ...
  $('.table > tbody > tr > td:first-child > input[type="checkbox"]').change(
    function () {
      var check = false;
      if ($(this).is(":checked")) {
        check = true;
      ...
      if (!check) {
        $('.table > thead > tr > th:first-child > input[type="checkbox"]').prop(
          "checked",
          false
        );
      }
